...
async function v1search(db, sdb, userInfo) {
    ...
    let sql = `SELECT sum(total_amount) as total,info_flag FROM sys_interface_status_es where interface_name='/v1/search' and token='${userInfo.token}' and ct ='${userInfo.time}' and info_flag in ('0101','0105','0109','02','03','04','0411','06','07','11','17','21','1201','1202','1301','1302') GROUP BY info_flag`
    let res = await sdb.query(sql)
    let list = res[0]
    if (list.length == 0) {
        return
    }
    let sql2 = `SELECT sum(total_amount) as total,info_flag FROM sys_interface_status_es where interface_name='/v1/search' and token='${userInfo.token}' and ct ='${userInfo.time}' and info_flag in ('0101','0105','0109','02','03','04','0411','06','07','11','17','21','1201','1202','1301','1302')`
    let res2 = await sdb.query(sql2)
    let total = res2[0][0].total
