...
let run = async function () {
    ...
    try {
        let users = await db.query(`select * from bill_move where is_erreo=0 and aid>1`) //and aid=9020686 
        users = users[0]
        for (const user of users) {
            ...
            for (const c of contract) {
              ...
              console.log(`进度：${user.aid} ${user.name} ${c.contract_id} 时间范围：${c.contract_start} - ${c.contract_end}`);
            }
        }
    } catch (error) {
        console.log(error);
