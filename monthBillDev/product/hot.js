...
async function child(db, baseInfo) {
    try {
        let sql = `select service_api from contract_detail where account_id=${baseInfo.aid} and order_detail_id='${baseInfo.order_detail_id}' and service_class->'$.value'=7 and is_deleted=0`
...
const fucntionClass = {
    ...
    async rank(db, baseInfo){
        ...
        try {
            let factor = await this.factor(db, baseInfo, '/api-dev/v3/hot/rank', 108)
    ...
    async search(db, baseInfo){
        ...
        try {
            let factor = await this.factor(db, baseInfo, '/api-dev/v3/hot/search', 126)
    ...
    async trend(db, baseInfo){
        ...
        try {
            let factor = await this.factor(db, baseInfo, '/api-dev/v3/hot/canvas/trend', 127)
